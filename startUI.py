...
if st.session_state.page == "home":
    ...
    if Type in ["일상 기록", "제품 소개", "칼럼"]:
        ...
        if Type == "일상 기록":
            ...
            st.session_state.info.append(mood)  # 0 idx

        brief_info = st.text_area("간략한 정보")
        st.session_state.info.append(brief_info)    # 1 idx
        tone = st.selectbox("말투", ["격식있는", "캐주얼한", "유머러스한", "CUSTOM"])
        st.session_state.info.append(tone)  # 2 idx
        ...
        if uploaded_file is not None:
            ...
            st.session_state.info.append(date_time) # 3 idx
...
#################### 중간 UI #######################
    ...
    if st.session_state.Type == "일상 기록":
        ...
        # 조건
        - 당시 기분은 {st.session_state.info[0]}
        - 여행 다녀온 날짜는 {st.session_state.info[3]}
        - 여행 블로그 글의 어투는 {st.session_state.info[2]}으로
        - (optional) {st.session_state.info[1]}이 사용자가 준 힌트 (비었다면 무시해도 됨.)
        - 반말을 기본으로
        ...
        (만약 음식 사진이라면, {st.session_state.info[0]}를 참고하여 음식의 맛에 대한 평가가 담긴 글이어야하고, 사용자가 간략한 정보를 줬다면, {st.session_state.info[1]}를 참고하여)
